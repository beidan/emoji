...
Emoji.prototype = {
    ...
    bindEvent: function () {
        ...
        $(document).on('click', '#emojiBox img', function () {
            var textarea = $('#' + _this.textareaId);
            $(this).clone().appendTo(textarea);
        });
        /*离开时收起框框*/
        emojiBox.hover(function () {
        }, function () {
            emojiBox.addClass('hide');
        });
        //控制展开收起
        $(document).on('click', '#' + _this.loadId + '', function () {
        ...
        /*发送数据*/
        $(document).on('click', '#' + _this.sendId + '', function () {
            var msg = _this.toText();
            console.log(msg);
        });
    },
    ...
    toEmoji: function () {
        ...
        var regex = new RegExp(':(' + keys + '):', 'g'),
            obj = $('.' + this.emojiTranslateCls),
            len = obj.length, i;
        for (i = 0; i < len; i++) {
            $(obj[i]).html($(obj[i]).html().replace(regex, emoji));
        }
        function emoji() {
            var key = arguments[1];
            return '<img src="" alt="' + key + '"class="emoji emoji_' + key + '"/>';
        }
    },
    /*转换成文字*/
    toText: function () {
        var textAreaObj = $('#' + this.textareaId), len, img,
            content = textAreaObj.html(),
            imgObj = [],
            i, j,
            format = ':';
        if (textAreaObj.find('img')) {
            img = textAreaObj.find('img'),
                len = img.length;
        } else {
            len = 0;
            img = [];
        ...
        /*获取img的alt属性*/
        for (i = 0; i < len; i++) {
            imgObj.push(format + img[i].getAttribute("alt") + format);
        }
        var length = imgObj.length;
        for (j = 0; j < length; j++) {
            content = content.replace(/<img\b[^>]*/, imgObj[j]);
        }
    ...
    loadEmoji: function () {
        ...
        emojiChoose.setAttribute('class', 'hide');
        document.getElementById(this.emojiContainer).appendChild(emojiChoose);
    }
}